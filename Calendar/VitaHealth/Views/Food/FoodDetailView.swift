...
//  VitaHealth
//
//  Re-styled to match RecipeEditorSheetView:
//  • `Form` + `Section` hierarchy
//  • Inline navigation title
//  • Cover image moved to its own section & extracted as sub-view
//  No business logic was changed. — 2025-05-19
//

import SwiftUI
import SwiftData
import PhotosUI          // за PhotosPicker
...
struct FoodDetailView: View {
    ...
    // nil → създаваме нова храна
    private let editingFood: Food?

    // ─────────────────────────────────────────────────────────
    // MARK: – Initial values (за isModified)
    // ─────────────────────────────────────────────────────────
...
//        initialProts    = prots
        ...
        // — cover image —
        if let data = food?.coverImage, let img = UIImage(data: data) {
...
//                    LabeledField(label: "Proteins (g)", text: $proteins)
            ...
            .navigationTitle(editingFood == nil ? "Add Food" : "Edit Food")
